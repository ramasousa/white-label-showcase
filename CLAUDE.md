...
| `baas-confianca-federacao-kyp.html` | **Confiança na federação, KYP e cláusulas**: a resposta à objeção de que o modelo federado (parceiro é o IdP) seria mais frágil que o standalone. Traz a matriz de assimetria de risco em 9 dimensões — o federado só perde em *uma*, a garantia herdada da autenticação —, as 6 fragilidades reais do federado (`R-01`…`R-06`, três delas sem correção técnica possível), os controles que as limitam (piso de `acr` no Token Exchange, step-up RFC 9470, SSF/CAEP, telemetria por parceiro), a tese central — **a autoridade do login é delegada, a de contratar nunca** —, o programa de *Know Your Partner* (tiering, questionário de identidade que vira parâmetro de plataforma, recertificação, kill switch) e a minuta de 12 cláusulas de contrato (`CL-01`…`CL-12`). Fecha com 19 referências normativas e 6 decisões em aberto (`G-01`…`G-06`). Paleta Velo. |
